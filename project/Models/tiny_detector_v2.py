...
class COCODetectionDataset(Dataset):
    def __init__(self, img_dir, ann_path, S=14, transform=None):
    ...
    def __getitem__(self, idx):
        ...
        label = torch.zeros((self.S, self.S, 5))
...
class YoloLoss(nn.Module):
    def __init__(self, S=14, lambda_coord=5, lambda_noobj=0.5):
        super().__init__()
        self.S = S
    ...
    def forward(self, preds, targets):
        ...
        return self.lambda_coord * coord_loss + loss_obj + self.lambda_noobj * loss_noobj
...
def evaluate(model, dataloader, device):
    ...
    with torch.no_grad():
        ...
        print(f"Detected {detected}/{int(total)} chocolates (≈{(detected/total)*100:.1f}%)")
            ...
            for y in range(S):
                for x in range(S):
                    ...
                    if obj_score > threshold:
                        ...
                        draw.text((x1, y1), f"Chocolate", fill="green")
...
if __name__ == "__main__":
    ...
    train_ds = COCODetectionDataset(TRAIN_IMG_DIR, TRAIN_JSON)
    val_ds = COCODetectionDataset(VAL_IMG_DIR, VAL_JSON)

    train_loader = DataLoader(train_ds, batch_size=8, shuffle=True, num_workers=2)
    val_loader = DataLoader(val_ds, batch_size=4)

    model = TinyDetector()
    if os.path.exists("tiny_detector_v2.pth"):
        model.load_state_dict(torch.load("tiny_detector_v2.pth"))
        print("Loaded pre-trained model.")


    train_model(model, train_loader, val_loader, device, epochs=10)
    torch.save(model.state_dict(), 'tiny_detector_v2.pth')
